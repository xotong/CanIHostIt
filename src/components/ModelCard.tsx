    ...
    >
      ...
      {!isExpanded && (
        <div className="grid grid-cols-4 gap-2 text-center">
          <div>
            <p className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>GPU</p>
            <p className="text-sm font-semibold truncate" style={{ color: 'var(--color-text-primary)' }}>
              {assignedGpu ? assignedGpu.name : 'Unassigned'}
            </p>
          </div>
          <div>
            <p className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>Batch</p>
            <p className="text-sm font-bold" style={{ color: 'var(--color-accent-emerald)' }}>
              {results ? results.effectiveBatchSize : '—'}
            </p>
          </div>
          <div>
            <p className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>TP×PP</p>
            <p className="text-sm font-bold" style={{ color: 'var(--color-accent-cyan)' }}>
              {results ? `${results.tpSize}×${results.ppSize}` : '—'}
            </p>
          </div>
          <div>
            <p className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>GPUs</p>
            <p className="text-sm font-bold" style={{ color: 'var(--color-text-primary)' }}>
              {results ? results.totalGpus : '—'}
            </p>
          </div>
        </div>
      )}
